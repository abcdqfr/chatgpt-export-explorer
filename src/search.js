...
async function handleGlobalSearch(query, onResults) {
  try {
    ...
    // Get export IDs from window.chatApp if available
    const exportIds = window.chatApp?.activeExports || [];
    const results = await dbOperations.globalSearch(query, exportIds);
...
async function handleConversationSearch(query, onResults) {
  try {
    ...
    const results = await dbOperations.searchInConversation(
...
function getCurrentConversation() {
  ...
  const { conversationId } = activeItem.dataset;
...
export function highlightText(text, query) {
  ...
  try {
    ...
    const sanitizedText = DOMPurify.sanitize(text, {
      ALLOWED_TAGS: ['p', 'br', 'strong', 'em', 'code', 'pre', 'a', 'span'],
    ...
    function highlightInNode(node, searchText) {
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        if (regex.test(content)) {
          ...
          const sanitizedHighlight = DOMPurify.sanitize(highlightedContent, {
            ALLOWED_TAGS: ['span'],
            ALLOWED_ATTR: ['class']
          });
          
          const tempContainer = document.createElement('div');
          tempContainer.innerHTML = sanitizedHighlight;
    ...
    return DOMPurify.sanitize(result, {
      ALLOWED_TAGS: ['span', 'p', 'br', 'strong', 'em', 'code', 'pre', 'a'],
      ALLOWED_ATTR: ['class', 'href', 'target', 'rel']
    });
  } catch (error) {
    console.error('Error highlighting text:', error);
    // Fallback: escape HTML and highlight
